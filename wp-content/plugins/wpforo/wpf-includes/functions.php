...
/**
 * @deprecated since 1.6.6 do not use this function it will be removed in next versions
...
*
* @return	void      to be echo html table
*/
function wpforo_create_form_table($varname, $primary_key, $fields = array(), $search_fields = array(), $filter_fields = array(), $actions = array(), $bulk_actions = array() ){
	...
	if(!empty($fields)) :
        ...
        <?php if($varname == 'moderation'): ?>
			...
			?>
            <ul class="subsubsub" style="margin:-25px 0 5px 0;">
							        ...
							        if( $userids = WPF()->db->get_col($sql) ) :  ?>
                                            <select name="filter_by_userid">
                                                ...
                                                <?php  foreach ($userids as $userid) : $user = WPF()->member->get_member($userid); ?>
                                                    <option value="<?php echo esc_attr($userid) ?>" <?php echo ( isset($_GET['filter_by_userid']) && $_GET['filter_by_userid'] == $userid ? 'selected' : '' ) ?>><?php echo esc_html( wpforo_user_dname($user) ) ?></option>
                                                <?php endforeach; ?>
					...
					<tr>
						...
						<?php foreach($fields as $key => $field) : ?>
							<th scope="col" <?php if($key===0) echo ' style="width:30%"' ?> id="<?php echo esc_attr(sanitize_text_field($field)) ?>" class="manage-column column-<?php echo esc_attr(sanitize_text_field($field)) ?> <?php echo isset($_GET['order']) ? ($_GET['orderby'] == $field ? 'sorted ' : '') . $_GET['order'] : 'sortable asc' ?>">
                                <?php if( in_array($field, array('blog_posts', 'blog_comments')) ) : ?>
													...
													<?php foreach($actions as $act_key => $action) : ?>
														<?php switch($action) :
															...
															case 'ban': if($item['userid'] == WPF()->current_userid) break; ?>
                                                            	<?php $member_status = wpfval($_GET, 'member_status');
                                                            	$ban_url = wp_nonce_url(
                                                            		admin_url( 'admin.php?page=wpforo-' . esc_attr(sanitize_text_field($varname)) . 's'. ($member_status ? '&member_status=' . $member_status : '') .'&id=' . intval($item[$primary_key]) . '&action=' . ($item['status'] == 'banned' ? 'unban' : 'ban') ), 'wpforo_admin_table_action_ban' ); ?>
											...
											<td>
												...
												<?php elseif($field == 'userid') : ?>
													<?php if(isset($item['name'])): ?>
                                                    	<?php $data = wpforo_member($item); echo esc_html(wpforo_user_dname($data)); ?>
                                                    <?php else: ?>
                                                    	<?php $data = WPF()->member->get_member($item[$field]); echo esc_html(wpforo_user_dname($data)); ?>
                                                    <?php endif; ?>
                                                ...
                                                <?php elseif( $field === 'blog_posts' ) : ?>
													<?php echo WPF()->member->blog_posts($item['userid']); ?>
                                                <?php elseif( $field === 'blog_comments' ) : ?>
													<?php echo WPF()->member->blog_comments($item['userid'], $item['user_email']) ?>
                                                <?php else : ?>
