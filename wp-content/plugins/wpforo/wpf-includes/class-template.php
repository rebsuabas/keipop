	...
	*/
	...
	public function member_buttons( $member ){
		...
		if( $profile_access ){
            ...
            <?php if( $member['userid'] == WPF()->current_userid ): ?>
                <a class="wpf-member-profile-button" title="<?php wpforo_phrase('Account') ?>" href="<?php echo esc_url(WPF()->member->profile_url($member, 'account')) ?>">
	...
	function init_nav_menu(){
		WPF()->menu['wpforo-home'] = array(
			...
			'attr'      => in_array( WPF()->current_object['template'], array(
				'forum',
				'topic',
				'post'
			) ) ? ' class="wpforo-active"' : '',
        ...
        if(WPF()->perm->usergroup_can('vmem')){
	        WPF()->menu['wpforo-members'] = array(
		        ...
		        'attr'      => WPF()->current_object['template'] === 'members' ? ' class="wpforo-active"' : '',
		        'submenues' => array()
	        );
        }
		...
		WPF()->menu['wpforo-recent'] = array(
			...
			'attr'      => ( ! wpfval( WPF()->GET, 'view' ) && WPF()->current_object['template'] === 'recent' ) ? ' class="wpforo-active"' : '',
			'submenues' => array()
		);
		...
		WPF()->menu['wpforo-unread'] = array(
			...
			'attr'      => wpfval( WPF()->GET, 'view' ) === 'unread' ? ' class="wpforo-active"' : '',
			'submenues' => array()
		);
		...
		WPF()->menu['wpforo-tags'] = array(
			...
			'attr'      => WPF()->current_object['template'] === 'tags' ? ' class="wpforo-active"' : '',
			'submenues' => array()
		);

        if( $member_id = WPF()->current_userid ){
	        ...
	        WPF()->menu['wpforo-profile-home']          = array(
		        ...
		        'attr'      => ( wpfval( WPF()->member_tpls, WPF()->current_object['template'] ) && WPF()->current_object['user_is_same_current_user'] ) ? ' class="wpforo-active"' : '',
		        'submenues' => array()
	        );
	        WPF()->menu['wpforo-profile-account']       = array(
		        'href'      => $url_account,
		        'label'     => wpforo_phrase( 'account', false ),
		        'attr'      => ( WPF()->current_object['template'] === 'account' && WPF()->current_object['user_is_same_current_user'] ) ? ' class="wpforo-active"' : '',
		        'submenues' => array()
	        );
	        WPF()->menu['wpforo-profile-activity']      = array(
		        'href'      => $url_activity,
		        'label'     => wpforo_phrase( 'activity', false ),
		        'attr'      => ( WPF()->current_object['template'] === 'activity' && WPF()->current_object['user_is_same_current_user'] ) ? ' class="wpforo-active"' : '',
		        'submenues' => array()
	        );
	        WPF()->menu['wpforo-profile-subscriptions'] = array(
		        'href'      => $url_subscriptions,
		        'label'     => wpforo_phrase( 'subscriptions', false ),
		        'attr'      => ( WPF()->current_object['template'] === 'subscriptions' && WPF()->current_object['user_is_same_current_user'] ) ? ' class="wpforo-active"' : '',
		        'submenues' => array()
	        );
	        ...
	        if ( wpforo_feature( 'user-register' ) ) {
		        WPF()->menu['wpforo-register'] = array(
			        ...
			        'attr'      => ( wpfval( WPF()->GET, 'wpforo' ) === 'signup' || wpfval( WPF()->GET, 'foro' ) === 'signup' ) ? ' class="wpforo-active"' : '',
			        'submenues' => array()
		        );
	        }
	        WPF()->menu['wpforo-login'] = array(
		        ...
		        'attr'      => ( wpfval( WPF()->GET, 'wpforo' ) === 'signin' || wpfval( WPF()->GET, 'foro' ) === 'signin' ) ? ' class="wpforo-active"' : '',
		        'submenues' => array()
	        );
        }
